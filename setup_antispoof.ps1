# setup_antispoof_fixed.ps1
# Run this script in: D:\Users\Lenovo\Desktop\考勤系统

$ErrorActionPreference = "Stop"
[Console]::OutputEncoding = [System.Text.Encoding]::UTF8

Write-Host "==== Setup MiniFASNetV2 ONNX Anti-spoofing ====" -ForegroundColor Cyan

$Root = (Get-Location).Path
$Backend = Join-Path $Root "backend"
$PatchPy = Join-Path $Root "patch_antispoof.py"
$ModelDir = Join-Path $Backend "models\anti_spoofing"
$ModelPath = Join-Path $ModelDir "MiniFASNetV2.onnx"
$ModelUrl = "https://github.com/yakhyo/face-anti-spoofing/releases/download/weights/MiniFASNetV2.onnx"

if (-not (Test-Path $Backend)) {
    throw "Cannot find backend folder. Please run this script in project root: D:\Users\Lenovo\Desktop\考勤系统"
}
...
if ($UseConda) {
    $envs = conda env list
    if ($envs -match "^\s*$EnvName\s") {
        Write-Host "Using conda env: $EnvName" -ForegroundColor Green
        $PythonPrefix = "conda run -n $EnvName python"
    } else {
        Write-Host "Conda env '$EnvName' not found. Creating it..." -ForegroundColor Yellow
        conda create -n $EnvName python=3.10 -y
        if ($LASTEXITCODE -ne 0) { throw "Failed to create conda env." }
        $PythonPrefix = "conda run -n $EnvName python"
    ...
    Write-Host "Conda not found. Using current Python." -ForegroundColor Yellow
    $PythonPrefix = "python"
}

Write-Host "Installing onnxruntime and opencv-python-headless..." -ForegroundColor Yellow
if ($UseConda) {
    conda run -n $EnvName python -m pip install onnxruntime opencv-python-headless
} else {
    python -m pip install onnxruntime opencv-python-headless
}
if ($LASTEXITCODE -ne 0) { throw "Failed to install Python packages." }

Write-Host "Downloading MiniFASNetV2.onnx..." -ForegroundColor Yellow
New-Item -ItemType Directory -Force $ModelDir | Out-Null

$NeedDownload = $true
if (Test-Path $ModelPath) {
    if ((Get-Item $ModelPath).Length -gt 100000) {
        $NeedDownload = $false
...
if (-not (Test-Path $ModelPath)) {
    throw "Model download failed: $ModelPath"
}

if ((Get-Item $ModelPath).Length -lt 100000) {
    throw "Downloaded model file is too small. Please delete it and rerun this script."
}

Write-Host "Writing Python patch script..." -ForegroundColor Yellow
...
Write-Host "Running Python patch..." -ForegroundColor Yellow
if ($UseConda) {
    conda run -n $EnvName python $PatchPy
} else {
    python $PatchPy
}
if ($LASTEXITCODE -ne 0) { throw "Python patch failed." }

Write-Host "Testing anti-spoofing model status..." -ForegroundColor Yellow
Push-Location $Backend
if ($UseConda) {
    conda run -n $EnvName python .\test_antispoof_status.py
} else {
    python .\test_antispoof_status.py
}
Pop-Location

if ($LASTEXITCODE -ne 0) {
    throw "Status test failed. Please check the printed last_error."
}

Write-Host ""
Write-Host "==== Done ====" -ForegroundColor Green
Write-Host "Now start backend:"
Write-Host "cd .\backend"
Write-Host "conda activate kaoqin"
Write-Host "python run.py"
Write-Host ""
Write-Host "Then open:"
Write-Host "http://127.0.0.1:8000/api/v1/system/model-status"